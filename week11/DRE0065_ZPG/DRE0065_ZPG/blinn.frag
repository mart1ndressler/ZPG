...
void main()
{
    ...
    float H = material.h; /*(material.h > 0.0) ? material.h : 128.0;*/
    if(sceneId == 2)
    {
        ...
        for(int i = 0; i < numberOfLights; i++)
        {
            ...
            float s = (d > 0.0) ? pow(max(dot(N, Hi), 0.0), H) : 0.0;
    ...
    if(forestLightMode == 1 || forestLightMode == 3)
    {
        for(int i = 0; i < numberOfLights; i++)
        {
            ...
            float s = (d > 0.0) ? pow(max(dot(N, Hvec), 0.0), H) : 0.0;
            ...
            specular += vec3(uLights[i].specular) * s * Rs * att * 0.50;
        }
    }
    ...
    if(flashlightOn == 1)
    {
        ...
        specular += flashlightDiffuse * s * Rs * att * spot * 0.50;
    }
