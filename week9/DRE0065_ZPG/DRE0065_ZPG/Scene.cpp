...
void Scene::handleClick(int mouseX, int mouseY, int width, int height)
{
    ...
    fprintf(stderr, "[PICK] Clicked pixel(%d, %d -> newY=%d), ""color=%02hhx%02hhx%02hhx%02hhx, depth=%f, stencil index=%u, object=%s\n",x, y, newY, color[0], color[1], color[2], color[3], depth, index, objectName);
}
...
void Scene::setupScene4()
{
	...
	textures.push_back(sunTexture); textures.push_back(earthTexture); textures.push_back(moonTexture);
...
void Scene::draw()
{
    ...
    else if(currentScene == 4)
    {
        ...
        DrawableObject* solarObjects[3] = {sunObject, earthObject, moonObject};
        Texture* solarTextures[3] = {sunTexture, earthTexture, moonTexture};
        for(int i = 0; i < 3; i++)
        {
            mat4 M_solar(1.0f);
