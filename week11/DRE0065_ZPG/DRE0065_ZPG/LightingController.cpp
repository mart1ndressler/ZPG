...
void LightingController::applyL(ShaderProgram* shaderProgram, int currentScene, int scene2LightsCount, int forestMode, const vec3* fireflyPos, int fireflyCount, vec3 camPos, vec3 camDir, bool flashlightOn, const vec3* objectColor, const float* emissiveVal)
{
    ...
    int lights = 16;
    float emissive = (emissiveVal ? *emissiveVal : 0.0f);

    shaderProgram->set("emissive", emissive);
    shaderProgram->set("sceneId", currentScene);
    shaderProgram->set("numberOfLights", 0);
    shaderProgram->set("forestLightMode", 0);
    shaderProgram->set("flashlightOn", 0);
    if(objectColor) shaderProgram->set("baseColor", *objectColor);

    if(currentScene == 2)
    {
        int useLights = std::max(0, std::min(scene2LightsCount, 5));
        shaderProgram->set("forestLightMode", useLights > 0 ? 1 : 0);
        shaderProgram->set("numberOfLights", useLights);
        ...
        for(int i = 0; i < useLights; i++)
        {
            ...
            snprintf(n, sizeof(n), "uLights[%d].atten", i); shaderProgram->set(n, vec3(1.0f, 0.0f, 0.0f));
        }

        if(useLights > 0) shaderProgram->set("lightPosition", scene2Fixed[0]);
        shaderProgram->set("dirLightDir", vec3(0.0f));
        shaderProgram->set("dirLightDiffuse", vec3(0.0f));
    }
    else if(currentScene == 3)
    {
        shaderProgram->set("forestLightMode", forestMode);
        int usePoints = (forestMode == 1 || forestMode == 3) ? std::max(0, std::min(std::min(fireflyCount, fireflyCap), lights)) : 0;
    ...
    {
        shaderProgram->set("forestLightMode", 1);
        shaderProgram->set("numberOfLights", 1);

        vec3 lp = vec3(0.0f);
        if(pointLight) lp = pointLight->getPosition();

        char n[64];
        snprintf(n, sizeof(n), "uLights[0].position"); shaderProgram->set(n, vec4(lp, 1.0f));
        snprintf(n, sizeof(n), "uLights[0].diffuse"); shaderProgram->set(n, vec4(1.0f));
        snprintf(n, sizeof(n), "uLights[0].specular"); shaderProgram->set(n, vec4(1.0f));
        snprintf(n, sizeof(n), "uLights[0].atten"); shaderProgram->set(n, vec3(1.0f, 0.18f, 0.1f));
