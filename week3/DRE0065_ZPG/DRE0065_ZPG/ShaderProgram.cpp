...
GLint ShaderProgram::uniformLoc(const char* name) const {return glGetUniformLocation(programID, name);}
void ShaderProgram::set(const char* name, int value) const {glUniform1i(uniformLoc(name), value);}
void ShaderProgram::set(const char* name, float value) const {glUniform1f(uniformLoc(name), value);}
void ShaderProgram::set(const char* name, const vec3& vec) const {glUniform3fv(uniformLoc(name), 1, &vec[0]);}
void ShaderProgram::set(const char* name, const mat4& mat) const {glUniformMatrix4fv(uniformLoc(name), 1, GL_FALSE, &mat[0][0]);}