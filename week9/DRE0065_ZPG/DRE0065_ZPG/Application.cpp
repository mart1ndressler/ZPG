...
void Application::handleInput()
{
    ...
    if(glfwGetKey(window, GLFW_KEY_R) == GLFW_PRESS && !rWasDown && scene->getScene() == 3) scene->skyboxOn = !scene->skyboxOn;
...
void Application::run()
{
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        if(glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS && !leftWasDown && scene) scene->handleClick((int)xpos, (int)ypos, w, h);
